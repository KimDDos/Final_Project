...
<div class="rvContainer">
	...
	<form class="rvForm">
		...
		<label class="rvLabel" for="rvName">이름:</label> <input class="rvInput"
			type="text" name="name" required> <label class="rvLabel"
			for="rvContact">연락처:</label> <input class="rvInput" type="text"
			name="contact" required> <label class="rvLabel" for="rvEmail">이메일:</label>
		<input class="rvInput" type="email" name="email" required> <label
			for="rvDate">날짜 선택:</label> <input type="text" class="rvInput"
			id="rvDate" name="date" required> <label class="rvLabel"
			for="rvTime">시간 선택:</label> <input class="rvInput" type="time"
			name="time" required>

		<div class="rvSelectedItemsContainer">
			<label class="rvLabel" for="rvItem">예약 항목:</label>
			<div class="rvCheckboxGroup">
				<label><input type="checkbox" name="reservationItems"
					value="item1" class="rvCheckbox">체중 감소/증가/관리</label> <label><input
					type="checkbox" name="reservationItems" value="item2"
					class="rvCheckbox">근성장</label> <label><input
					type="checkbox" name="reservationItems" value="item3"
					class="rvCheckbox">체형교정</label> <label><input
					type="checkbox" name="reservationItems" value="item4"
					class="rvCheckbox">건강</label> <label><input type="checkbox"
					name="reservationItems" value="item5" class="rvCheckbox">체지방
					집중관리</label> <label><input type="checkbox" name="reservationItems"
					value="item6" class="rvCheckbox">재활치료</label>
			</div>
		</div>
	...
	$(document)
			.ready(
					function() {
						...
						// 항목 삭제 버튼 클릭 이벤트
						$('#rvSelectedItemsList').on(
								'click',
								'.removeItem',
								function() {
									var valueToRemove = $(this).data('value');
									$("input[value='" + valueToRemove + "']")
											.prop('checked', false).change(); // 해당 체크박스 선택 해제
								});
					});
</script>
<script>
